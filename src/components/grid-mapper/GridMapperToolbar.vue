        ...
        <div class="flex gap-1">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start" class="w-80">
              ...
              <div v-if="versionManager.versions.value.length === 0" class="text-xs text-muted-foreground text-center py-4">
                No saved versions yet
              </div>

              <div v-else class="max-h-[300px] overflow-y-auto">
                <div v-for="version in versionManager.versions.value" :key="version.id" class="p-2 border-b last:border-b-0">
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        ...
                        <AlertDialogFooter>
                          ...
                          <AlertDialogAction @click="handleDeleteVersion(version.id)">
                            Delete
                          </AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
...
import { Link, X, Grid3x3, Maximize, Download, FileJson, ImageIcon, ImageOff, Trash2, Save } from "lucide-vue-next"
...
const abilityNameToId = abilitiesData.reduce((map, ability) => {
...
function handleExport() {
  ...
  const exportedNodes = ctx.nodes.value.map((node) => {
    const connectedNodes = node.connections.map((connectedId) => {
